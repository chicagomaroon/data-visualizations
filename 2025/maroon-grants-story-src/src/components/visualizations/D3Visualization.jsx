...
const D3Visualization = ({ currentStepIndex, direction, highlighted }) => {
    ...
    useEffect(() => {
        ...
        const tooltip = d3
            ...
            .style('z-index', '1000');
        d3.csv(`${import.meta.env.BASE_URL}data/data.csv`)
            .then((data) => {
                data.forEach((d) => {
                ...
                const bottomCategories = [
                    [
                        ['infectious diseases and COVID-19', 0, 0.3],
                        ['health disparities and accessibility', 0, 0.3]
                    ],
                    [
                ...
                const root = d3
                    .stratify()
                    .id((d) => d.Title)
                    .parentId((d) => d.Category)(hierarchyData);

                // Calculate values
                root.sum((d) => d.Grant_Amount) // give every node at least value 1
                ...
                outerRectsRef.current = g
                    ...
                    .attr('x', (d) => {
                        const matchingCategory = bottomCategories
                            .flat()
                            .find((c) => c[0] === d.data.Category);
                        const title = d.data.Title.toLowerCase();
                        const buffer2 = title.includes(
                            'spatiotemporal models of neural'
                        )
                            ? 10
                            : 0;
                        const buffer3 = title.includes('designing access')
                            ? 25
                            : 0;
                        const buffer4 = title.includes('spatiotemporal models')
                            ? -0.3
                            : 0;
                        const buffer = buffer2 + buffer3 + buffer4;
                        return matchingCategory
                            ? matchingCategory[1] + buffer
                            : d.x0;
                    })
                    .attr('y', (d) => {
                        const matchingCategory = bottomCategories
                ...
                innerRectsRef.current = g
                    ...
                    .attr('width', (d) => {
                        const matchingCategory = bottomCategories
                            ...
                            .find((c) => c[0] === d.data.Category);
                        return matchingCategory
                            ? d.data.Actual_Amount / 10000
                            : (d.data.Actual_Amount / d.data.Grant_Amount) *
                                  (d.x1 - d.x0);
                    })
                    .attr('height', (d) => {
                        const matchingCategory = bottomCategories
                ...
                grantTitlesRef.current = g
                    ...
                    .each(function (d) {
                        ...
                        if (title.includes('Understanding Political Economy')) {
                            ...
                            wrapText(text, title, 2.8, width, 0.55, -7.2);
                            return title;
                    ...
                    .attr('fill', (d) => {
                        ...
                        return title.includes('Spatiotemporal Models') ||
                            title.includes('Understanding Political Economy') ||
                            title.includes('Understanding the Impact')
                            ? 'white'
                            : 'gray';
                    })
                    .style('opacity', 0);
                ...
                grantValuesRef.current = g
                    .selectAll('text.grant-value')
                    .data(root.leaves())
                    .join('text')
                    ...
                    .attr('font-size', (d) => {
                        ...
                        return matchingCategory
                            ? 0
                            : Math.min(12, (d.x1 - d.x0) / 5);
                    })
                    .attr('fill', (d) => {
                        const cat = d.data.Category.toLowerCase();
                        return cat.includes('infectious') ||
                ...
                grantNewValuesRef.current = g
                    .selectAll('text.actual-value')
                    .data(root.leaves())
                    .join('text')
                    .attr('class', 'grant-value')
                    .attr('x', (d) => {
                    ...
                    .text((d) => {
                        ...
                        return (
                            '-' +
                            (d.data.Lost_Amount / 1000000).toFixed(2) +
                            'M'
                        );
                    })
                    .attr('font-size', (d) => {
                ...
                categoryTitlesRef.current = g
                    ...
                    .attr('y', (d) => {
                        const matchingCategory = bottomCategories
                            ...
                            .find((c) => c[0] === d.data.Title);
                        return matchingCategory
                            ? 655 + matchingCategory[2] * 50
                            : d.y0 + 20;
                    })
                    .text((d) => {
                        if (d.data.Title.toLowerCase().includes('covid'))
                            return 'Infectious Diseases and COVID-19';
                        if (d.data.Title.toLowerCase().includes('infectious'))
                            return 'Non-Infectious and Infectious Diseases';
                ...
                categoryTotalsRef.current = g
                    .selectAll('category-totals')
                    .data(root.children)
                    .enter()
                    .append('text')
                    .attr('x', (d) => {
    ...
    const setOpacity = (
        ...
        if (title === '') {
            ...
            selectionRef.current
                ...
                .duration(duration)
                .style('opacity', (d) =>
                    d.data.Title.includes(title) ? trueValue : falseValue
                );
        }
    };

    const setFontAndOpacity = (
        selectionRef,
        title,
        falseValue,
        duration,
    ...
    const stateEffects = (step, direction, duration = 2000) => {
        ...
        if (step < 3) {
            ...
            innerRectChange(outerRectsRef, 0.3, duration);
            ...
            innerRectChange(outerRectsRef, 0.1, duration);
            innerRectChange(innerRectsRef, 0.1, duration);
