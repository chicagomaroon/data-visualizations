...
const dataFirst = dataAll.features.filter(
    (d) => d.properties.year_start <= 1925
);

// -------- MAP FUNCTIONS ---------
//fead in layer
function fadeInLayerLeaflet(
    layer,
    startOpacity,
    finalOpacity,
    opacityStep,
...
function waypoints() {
    ...
    new Waypoint({
        ...
        handler: function (direction) {
            if (direction == 'down') {
                ...
                mapIntro.flyTo([41.79139, -87.6], 15.5);
            }
        },
        offset: offset
    });
    ...
    new Waypoint({
        ...
        handler: function () {
            ...
            introAllBuildings = L.geoJSON(dataAll, {
                style: buildingStyleShow
            }).addTo(mapIntro);
            mapIntro.flyTo([41.79139, -87.6], 15, {
                animate: true,
                duration: 1
            });
        },
    ...
    for (const chapter of config) {
        for (const subsection of chapter.subsections) {
            ...
            new Waypoint({
                ...
                handler: function (direction) {
                    if (direction == 'down') {
                        const data = dataAll.features.filter(
                            (d) =>
                                d.properties.year_start >=
                                    subsection.start_year &&
                                d.properties.year_start <= subsection.end_year
                        );
                        //bodyAllBuildings.clearLayers();
                        layers[subsection.id] = L.geoJSON(data, {
                            style: buildingStyleShow
                        }).addTo(mapBody);
...
function main() {
    ...
    introFirstBuildings = L.geoJSON(dataFirst, {
        style: buildingStyleShow
    }).addTo(mapIntro);
